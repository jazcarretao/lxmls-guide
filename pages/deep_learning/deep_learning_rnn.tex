   ...
   \STATE {\bfseries input:} Input data matrix $\mathbf{x} \in \mathbb{R}^{I \times M'}$, representing sentence of $M'$ time steps. Initial recurrent variable $\mathbf{h}_0$. 
	...
	\FOR{$m=1$ {\bfseries to} $M'$}
     \STATE Apply embedding layer 
        $$m_{dm} = \sum_{i=1}^{I} W^e_{di} x_{dm}$$
     \STATE Apply linear transformation combining embedding and recurrent signals
        $$z_{jm}^h = \sum_{d=1}^{E} W_{jd}^x m_{dm} + \sum_{j'=1}^{J} W_{jj'}^h h_{j'm-1}$$
     \STATE Apply non-linear transformation e.g. sigmoid (hereby denoted $\sigma()$)
     $$h_{jm} = \sigma(z_{jm}^h)  = \frac{1}{1+\exp(-z_{jm}^h)}$$

	\ENDFOR

\STATE Apply final linear transformation to each of the recurrent variables $\mathbf{h}_1 \cdots \mathbf{h}_M'$ 
...
\begin{algorithmic}[1]
   ...
   \STATE initialize parameters $\Theta$ randomly
	...
	\FOR{$m=1$ {\bfseries to} $M'$}
        ...
        keep not only $\tilde{z}_{km}^y$ but also, the RNN layer activations $z_{jt}^h$ and the embedding layer activations $z^e_{di}$.
	\ENDFOR	
	\vspace{0.3cm}
    ...
    \STATE {Initialize the error at last layer, for each example $m$. For the softmax with CE cost this is given by:
        $$\mathbf{e}^{m} = \Big(\mathrm{\mathbf{1}}_{k(m)} - \tilde{\mathbf{z}}_{m}^y \Big).$$}
	\ENDFOR	

    \STATE Initialize recurrent layer error $$\mathbf{e}_h$$ to a vector of zeros
   	\FOR{$m=M'-1$ {\bfseries to} $2$}
	\STATE {Backpropagate} the error from output to recurrent layer:
    $$\mathbf{e}^{m-1} = \Big((\mathbf{W}^y)^T \mathbf{e}^{m}\Big) + \mathbf{e}^{m}$$
