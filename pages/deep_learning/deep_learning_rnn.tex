...
\begin{algorithmic}[1]
   ...
   \STATE initialize parameters $\Theta$ randomly
	...
	\FOR{$m=1$ {\bfseries to} $M'$}
        ...
        keep not only $\tilde{z}_{km}^y$ but also, the RNN layer activations $h_{jt}$ and the embedding layer activations $z^e_{di}$.
	\ENDFOR	
	\vspace{0.3cm}
	...
	\STATE where $\odot$ is the element-wise product and the $\mathbf{\mathrm{1}}$ is replicated to match the size of $\tilde{\mathbf{h}}^n$.
    ...
    $$\mathbf{e}^m_e = (\mathbf{W}^x)^T \mathbf{e}_h$$
    ...
    Take into account that for the embedding layer 
